...
def query_cube_view (config, view_definition):
	...
	cube_name = view_definition ['view']['cube']
	if view_definition['view']['type']=='native':
		view_name = view_definition ['view']['name']
	...
	elif view_definition['view']['type']=='mdx':
		mdx_query = view_definition['view']['mdx']
		view_name = "MDX query at %s" % (time.strftime("%Y%m%d-%H%M%S"))
		with TM1Service(**config[tm1_server_name], timeout=50) as tm1:
			cell_set = tm1.cubes.cells.execute_mdx(mdx_query)
	#elif view_definition['view']['type']=='random':
		# this is where we try to create a new view and somehow 'randomise it' by selecting different elements
		# we are going to record this view back as MDX view based on whatever we generated randomly
		# view_definition['view']['mdx'] = whatever we generate converted to mdx
	else: 
		logging.error("View type %s in test definition is not implemented" % (view_definition['view']['type']) )
...
def compare_cell_sets(check_file, cell_set_source, cell_set_target):
	...
	if (cell_set_source != cell_set_target):
		...
		for diff in dictdiffer.diff(cell_set_source, cell_set_target):
			#TODO: need to check the magnitude of difference, cause we might get 0.01 variances on large consolidations in different instances
			
			csv_writer.writerow({'change_type':diff[0], 'cell':diff[1],'source_value':diff[2][0], 'target_value':diff[2][1]})
			num_of_variances += 1
		logging.warning ("Source and target views do not match, %n cells vary, please find list of differences in the file %s" %(num_of_variances ,file_name_to_write_diff))
		return False
	else:
...
def run_check(input_file, config):
	...
	logging.info('Start running test %s'%(check_definition['test_name']))
